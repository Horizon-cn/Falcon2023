...
local playerDist2Ball = function(playerNum)
	return ball.valid() and player.pos(playerNum):dist(ball.pos()) or 9999
end
...
local enemyDist2Ball = function(enemyNum)
	return ball.valid() and enemy.pos(enemyNum):dist(ball.pos()) or 9999
end
...
local resultOutput = function(finished)
	if finished then
		debugEngine:gui_debug_msg(CGeoPoint:new_local(-400,-220),string.format("FINISHED"),0)
	end
	local BLUE_COUNT = IS_YELLOW and ENEMY_COUNT or MY_COUNT
	local YELLOW_COUNT = IS_YELLOW and MY_COUNT or ENEMY_COUNT
	debugEngine:gui_debug_msg(CGeoPoint:new_local(-400,-240),string.format("time : %4.2f",SUM_TIME - ALL_COUNT/FREQ))
	debugEngine:gui_debug_msg(CGeoPoint:new_local(-400,-260),string.format("BLUE : %4.2f",BLUE_COUNT/FREQ),6)
	debugEngine:gui_debug_msg(CGeoPoint:new_local(-400,-280),string.format("YELL : %4.2f",YELLOW_COUNT/FREQ),3)
end
local refRun = function(running)
	local playerCount = 0
	local enemyCount = 0
	local ballColor = getColor()
	debugEngine:gui_debug_arc(ball.pos(),400,0,360,ballColor)
	debugEngine:gui_debug_arc(ball.pos(),415,0,360,ballColor)
	debugEngine:gui_debug_arc(ball.pos(),430,0,360,ballColor)
	debugEngine:gui_debug_msg(ball.pos(),string.format("v: %.0f",ball.velMod()),ballColor)
	...
	for i = 0, param.maxPlayer - 1 do
		...
		if enemy.valid(i) then
			...
			debugEngine:gui_debug_msg(CGeoPoint:new_local(-100,param.pitchWidth/2-30-enemyCount*150),string.format("%02d : %4.0f",i,dist),color)
		end
	end
	...
	if running then
		...
		debugEngine:gui_debug_msg(CGeoPoint:new_local(-400,-220),string.format("PAUSEING"),0)
	end
	resultOutput(false)
end
