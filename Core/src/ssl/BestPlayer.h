...
public:
	...
	double getOurPotential(int num) { return _Potentials[num][0]; }
	const PlayerList& theirFastestPlayerToBallList() const { return _theirFastestPlayerToBallList; }
	double getTheirPotential(int num) const { return _theirPotentials[num][0]; }
	...
	bool isOurPlayerStrictControlBall(int num) { return _ourPlayerBallStrictControl[num]; }
	bool isOurPlayerLooseControlBall(int num) { return _ourPlayerBallLooseControl[num]; }

	bool isTheirPlayerStrictControlBall(int num) { return _theirPlayerBallStrictControl[num]; }
	bool isTheirPlayerLooseControlBall(int num) { return _theirPlayerBallLooseControl[num]; }
...
protected:
	...
	double velBallToPlayer(const PlayerVisionT& player,const BallVisionT& ball,const double *biases,double maxDist);//相对速度
private:
	/// 我方球员的势能
