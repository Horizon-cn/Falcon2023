...
void CGoAndTurnKickV3::plan(const CVisionModule* pVision) {
	...
	switch (state()) {
		...
		case GO:
			...
			break;
		case GOAROUND:
			if (CircleCounter >= 65*1.25 * circleNum && abs(Utils::Normalize((ball.Pos() - me.Pos()).dir() - finalDir)) < gotoPre) {
				new_state = ADJUST;
			}
			else {
				CircleCounter++;
			...
			break;
		case ADJUST:
			if (abs(Utils::Normalize(me.Dir() - finalDir)) < adjustPre || adjustCount >= 30*1.25) {
				new_state = KICK;
			}
			adjustCount++;
			CircleCounter = 0;
