...
void CForceStartRush::plan(const CVisionModule* pVision)
{
	...
	//}
	//GDebugEngine::Instance()->gui_debug_line(self.Pos(),self.Pos()+Utils::Polar2Vector(1000,adjustDir),COLOR_BLUE);
	//GDebugEngine::Instance()->gui_debug_line(self.Pos(),self.Pos()+Utils::Polar2Vector(1000,self.Dir()),COLOR_BLACK);
	TaskT rushTask(task());
