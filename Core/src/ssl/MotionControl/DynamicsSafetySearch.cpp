...
#define ACCURATE_AOID 250
	double MIN_TEAMMATE_AVOID_DIST = 1;
	double MAX_TEAMMATE_AVOID_DIST = 9;
	double TEAMMATE_AVOID_DIST = 9;
	double MIN_OPP_AVOID_DIST = 4;//  1
	double MAX_OPP_AVOID_DIST = 15;//  9
	double OPP_AVOID_DIST = 9;
	double MIN_BALL_AVOID_DIST = 3;
	double MAX_BALL_AVOID_DIST = 5;
	double BALL_AVOID_DIST = 3; // 厘米 1.5
	double MAX_TRANSLATION_SPEED = 350; // 350
	double MAX_TRANSLATION_ACC = 650;   // 650
	double MAX_TRANSLATION_DEC = 650;   // 650
	double MIN_SPEED_OPEN_DSS = 100;
}
/************************************************************************/
...
void CDynamicSafetySearch::refresh_priority(const int player, const int t_priority, const vector2f target, const vector2f pos) {
	...
	if (DEBUGFLAG) {
		cout<<"player:"<<player<<'\t'<<"priority: "<<priority[player]<<endl;
	}
}
/************************************************************************/
/* modified 2011-12-26 by HQS
...
bool CDynamicSafetySearch::CheckAccel (const int player, vector2f acc, const CVisionModule* pVision, vector2f& crash_car_dir, int& crash_car_num, int& type, const float limitTime) {
	...
	if(pVision->Ball().Valid() && (_flag & PlayerStatus::DODGE_BALL)) {
		...
		const BallVisionT& last_ball = pVision->Ball(pVision->LastCycle());
		vector2f Aj;
