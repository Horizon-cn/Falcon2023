...
void CBestPlayer::updateBothGoalie(const CVisionModule *pVision)
{
	...
	theirGoalie = pVision->TheirGoalie();
}
...
void CBestPlayer::updateBall(const CVisionModule* pVision)
{
	...
	for (int i = 0; i < Param::Field::MAX_PLAYER; i ++) {
		if (pVision->OurPlayer(i).Valid() && i != goalieNumber) {
			CVector playerToBall = BallPos- pVision->OurPlayer(i).Pos();
	...
	for (int i = 0; i < Param::Field::MAX_PLAYER; i ++) {
		...
		if (! ourPlayerValid) {
			_ourPlayerBallLooseControl[i] = false;
			_ourPlayerBallStrictControl[i] = false;
			continue ;
		}
		...
		if (ballLooseCtrl) {
			_ourPlayerBallLooseControl[i] = true;
		...
		if (ballStrictCtrl) {
			_ourPlayerBallStrictControl[i] = true;
	...
	for (int i = 0; i < Param::Field::MAX_PLAYER; i ++) {
		...
		if (! theirPlayerValid) {
			_theirPlayerBallLooseControl[i] = false;
			_theirPlayerBallStrictControl[i] = false;
			continue ;
		}
		...
		if (ballLooseCtrl) {
			_theirPlayerBallLooseControl[i] = true;
		...
		if (ballStrictCtrl) {
			_theirPlayerBallStrictControl[i] = true;
...
{	
	...
	// 进行更新
	double antiShootDir=Utils::Normalize((theirGoal-pVision->Ball().Pos()).dir()+Param::Math::PI);
	...
	for (int i = 0; i < Param::Field::MAX_PLAYER; ++ i) {
		if (pVision->OurPlayer(i).Valid()) {
			...
			CGeoPoint _ourPlayer = pVision->OurPlayer(i).Pos();
			...
			if (biases[0] != 0) {
				_Potentials[i][1] = distBallToPlayer(playerToKickPos,biases,MaxToBallDist);
				_Potentials[i][5] = velBallToPlayer(pVision->OurPlayer(i),pVision->Ball(),biases,MaxToBallDist);
				if(VERBOSE_MODE) cout << "Player: " << i  <<  "	"<<" "<<playerToBall.mod()<<" "<<_Potentials[i][1] <<"  "<< 	_Potentials[i][5]  << endl;
				//if (_Potentials[i-1][5]+_Potentials[i-1][1] < 0) {
				//	_Potentials[i-1][5] = -_Potentials[i-1][1];//保证这里不出现负值
				//}
			}
			if (biases[1] != 0) {
				_Potentials[i][2] = 0;//shootAngle(_ourPlayer,BallPos,biases,true);
			}
			if (biases[2] != 0) {
				_Potentials[i][3] = 0;//clearPath(pVision,_ourPlayer,BallPos,biases);
			}
			if (biases[3] != 0) {
				_Potentials[i][4] = 0;//anglePlayerToBall(pVision->OurPlayer(i),BallPos,biases);
			}
			//[1]distBallToPlayer
			//[2]shootAngle
			//[3]clearPath
			...
			_Potentials[i][6] = distFactor*_Potentials[i][1]+velFactor*_Potentials[i][5];
			_Potentials[i][0] = _Potentials[i][6];

			// 若是同上次一致，则需进行降低系数处理
			if (i == _ourLastBestPlayer) {
				//double multiAlpha = 0.8 + 0.05 * old_fastest_locate;
				_Potentials[i][0] -= 0.12;
			}
			...
			if (i != goalieNumber) {
				...
				for (int j=0;j<Param::Field::MAX_PLAYER;j++)
				{
					if (pVision->OurPlayer(j).Valid()&&j!=i){
						CGeoPoint proj=metoBallSeg.projection(pVision->OurPlayer(j).Pos());
						double dist = proj.dist(pVision->OurPlayer(j).Pos());
...
/*				if (_ourPlayerBallStrictControl[i-1]) {
				...
				_Potentials[i][0] = 1.5*_Potentials[i][0] + 1;
			}			
		} else {
			_Potentials[i][0] = OurPlayerLoseDefualtPotential;
		}

		// 综合前后两次的计算进行统一
		double current_potential = OurEvaluateAlpha * _Potentials[i][0] + (1.0-OurEvaluateAlpha) * _Last_Potentials[i][0];
		...
		_Potentials[i][0] = current_potential;
	}
	...
	//		CGeoPoint theirGoal=CGeoPoint(Param::Field::PITCH_LENGTH/2,0);
	//		double lasttoGoalDir=(theirGoal-pVision->OurPlayer(tempBestPlayer).Pos()).dir();
	//		double lasttoBallDir=(BallPos-pVision->OurPlayer(tempBestPlayer).Pos()).dir();
	//		double lastDiff=fabs(Utils::Normalize(lasttoBallDir-0));

	//		double patchertoGoalDir=(theirGoal-pVision->OurPlayer(patcherOne).Pos()).dir();
	//		double patchertoBallDir=(BallPos-pVision->OurPlayer(patcherOne).Pos()).dir();
...
void CBestPlayer::updateTheirPlayerList(const CVisionModule *pVision,const double *biases)
{
	...
	for (int i = 0; i < Param::Field::MAX_PLAYER; ++ i) {
		if (pVision->TheirPlayer(i).Valid()) {
			...
			if (tempBiases[0] != 0) {
				_theirPotentials[i][1] = Robot_Ability_Diff*distBallToPlayer(playerToBall,tempBiases,MaxToBallDist);
				_theirPotentials[1][5] = Robot_Ability_Diff*velBallToPlayer(pVision->TheirPlayer(i),pVision->Ball(),tempBiases,MaxToBallDist);
			...
			if (tempBiases[1] != 0) {
				_theirPotentials[i][2] = 0;//shootAngle(_theirPlayer,BallPos,tempBiases,false); //对方的球门和我们的相反，还没考虑
			}
			if (tempBiases[2] != 0) {
				_theirPotentials[i][3] = 0;//theirClearPath(pVision,_theirPlayer,BallPos,tempBiases);
			}
			if (tempBiases[3] != 0) {
				_theirPotentials[i][4] = 0;//anglePlayerToBall(pVision->TheirPlayer(i),BallPos,tempBiases);

			}
			_theirPotentials[i][0] = _theirPotentials[i][1]+_theirPotentials[i][2]+_theirPotentials[i][3]+_theirPotentials[i][4]+_theirPotentials[i][5];
			
			// 若是同上次一致，则需进行降低系数处理
			if (i == _theirLastBestPlayer) {
				double multiAlpha =  0.8 + 0.05 * old_their_fastest_locate;
				_theirPotentials[i][0] *= multiAlpha;
			}

			// TODO 不是很准确的
			if (i != theirGoalie) {
				if (_theirPlayerBallStrictControl[i]) {
					_theirPotentials[i][0] = TheirBallControlDefaultPotential;
				} else if (_theirPlayerBallLooseControl[i]) {
					_theirPotentials[i][0] = TheirBallControlDefaultPotential + 0.1;
				}				
			} else { // 门将的可能性要降低
				_theirPotentials[i][0] = 1.5*_theirPotentials[i][0] + 0.2;
			}		
		} else {
			_theirPotentials[i][0] = TheirPlayerLoseDefualtPotential;
		}

		double current_their_potential = TheirEvaluateAlpha * _theirPotentials[i][0] + (1.0-TheirEvaluateAlpha) * _Last_theirPotentials[i][0];
		...
		_theirPotentials[i][0] = current_their_potential;
...
double CBestPlayer::velBallToPlayer(const PlayerVisionT& player,const BallVisionT& ball,const double *biases,double maxDist)
{
	/// 限制球速大小为8m/s
