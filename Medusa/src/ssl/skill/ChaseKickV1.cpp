...
namespace
{
	...
	const double Ball_Moving_Fast_Speed = 50;	// 原来为1m/s，现在改为50cm/s
    //
	...
	CGeoPoint GOAL_LEFT_POS = CGeoPoint(Param::Field::PITCH_LENGTH / 2, -Param::Field::GOAL_WIDTH / 2 );    //对面球门上边沿
...
CChaseKickV1::CChaseKickV1()
{
	DECLARE_PARAM_READER_BEGIN(ADVANCE)
		READ_PARAM(CHASEKICK_TOPENALTY_GAP)
		READ_PARAM(CHASEKICK_ENDVEL_STARTX)
		READ_PARAM(CHASEKICK_MAXENDVEL)
...
void CChaseKickV1::plan(const CVisionModule* pVision)
{
	...
	const CVector self2goal = GOAL_CENTRE_POS - me.Pos();								//小车到球门中心向量
	const CVector ball2goal = GOAL_CENTRE_POS - ball.Pos();								//球到球门中心向量
	const CVector ball2goalLeft = GOAL_LEFT_POS - ball.Pos();						    //球到球门上边沿向量
	const CVector ball2goalRight = GOAL_RIGHT_POS - ball.Pos();						    //球到球门上边沿向量
	...
	const double reverse_BallVelDir = Utils::Normalize(ball.Vel().dir() - Param::Math::PI);		//球速方向的反向
	...
	bool isBallAtFront = fabs(Utils::Normalize(self2rawball.dir() - me.Dir())) < Param::Math::PI / 2.0;   //判断球是否在车前
	bool isBallAtDirectFront = fabs(Utils::Normalize(self2rawball.dir() - me.Dir())) < Param::Math::PI / 4.0;   //判断球是否在车的正前方，这时候车头可与球速同向，稳住球
	bool isBallToGoal = min(ball2goalLeft.dir(), ball2goalRight.dir()) < ball.Vel().dir() && ball.Vel().dir()< max(ball2goalLeft.dir(), ball2goalRight.dir()); //判断球是否朝球门去
	...
	const CGeoSegment ballMovingLongSeg = CGeoSegment(ball.Pos(), ball.Pos() + Utils::Polar2Vector(2000, Utils::Normalize(ballVelDir))); //长球速延申线段，用于判断追球or接球
	...
	const CGeoPoint LongprojMe = ballMovingLongSeg.projection(me.Pos());                //小车在球长移动线上的投影点
	...
	const bool meOnBallMovingLongSeg = ballMovingLongSeg.IsPointOnLineOnSegment(LongprojMe);	//投影点是否在球速长线段上面
	...
	const double ReceiveBallAngle = Utils::Normalize((self2goal.dir() + reverse_BallVelDir) / 2.0); //小车接球方向的夹角，为朝球门中心-球员-球速反方向夹角角平分线，利用镜面反射原理，尽可能控住球
	const double LargeReceiveBallAngle = Utils::Normalize(self2goal.dir() / 3.0 + 2 * reverse_BallVelDir / 3.0); //更趋向球速线的小车接球方向的夹角，在球迎面来的时候尽可能控住球
	...
	if (ballSpeed>=Ball_Moving_Fast_Speed)
	{
		...
		case RUSH_TO_BALL:
			...
			if (!meOnBallMovingLongSeg) {                               //车辆投影不在球速移动线上，追球阶段或离球较远
				...
				if (isBallAtFront) {        //球在车前
					if (real_predict_ballPos.x() > Param::Field::PITCH_LENGTH / 2 -  1.5 * queryParamByName("data\\ssl\\params\\params.xml", "PENALTY_AREA_DEPTH") ) {
					...
					if ( isBallToGoal) {
										//球往门框范围里去或者球几乎沿x轴方向运动,避开球，同之前避开球方向类似
					...
					else if (real_predict_ballPos.x() > Param::Field::PITCH_LENGTH / 2 - 1.5 * queryParamByName("data\\ssl\\params\\params.xml", "PENALTY_AREA_DEPTH")) {
	...
	else
	{
		...
		double end_vel = 0;
		double ENDVEL_ENDX = Param::Field::PITCH_LENGTH / 2 - queryParamByName("data\\ssl\\params\\params.xml", "PENALTY_AREA_DEPTH") - CHASEKICK_TOPENALTY_GAP;
