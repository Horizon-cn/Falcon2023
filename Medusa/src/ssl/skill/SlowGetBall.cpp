...
void CSlowGetBall::plan(const CVisionModule* pVision)
{
	...
	{
		if (getballComplete || isBallFrontOfMyhead && actionCount > 300*1.25)//************TODO解决不上前问题，此处先强硬处理
		{
			setState(S_GOTOWARD);
			cntJumpOutGoTowards = 0;
			actionCount = 0;
			startPoint = ball.Pos();
		} else if (actionCount > 60*1.25)
		{
			setState(BEGINNING);
			actionCount = 0;
		}
	...
	else if (S_GOTOWARD == getState())
	{
		...
		if (trueBallCatched || startPoint.dist(me.Pos()) < 6) //没有上传的时候 用视觉补偿
		{
			...
			if (actionCount > 150*1.25 && !fraredOn || !dirReached || !isBallFrontOfMyhead)//如果在向前进途中，折回拿球状态，注意与actionCount所比较的值有没有问题
			{
				if (++cntJumpOutGoTowards > 5*1.25)
				{				
					setState(BEGINNING);
					actionCount = 0;
				}
	...
	{
		// if (me.Pos().dist(ball.Pos()) > 50 /* || fraredLongOff > 30*/)
		if (true)
		{
			setState(BEGINNING);
			actionCount = 0;
	...
	//限制actionCount
	if (actionCount > 500*1.25)
	{
		actionCount = 500*1.25;
	}
