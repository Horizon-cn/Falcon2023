...
bool CEnemyDefendTacticArea::isInLongCircle(CGeoPoint & enemyCarPos)
{
	...
	{
		if ((enemyCarPos - theirGoal ).mod() <  ( queryParamByName("data\\ssl\\params\\params.xml", "PENALTY_AREA_DEPTH") - 10 ))
		{
			return true;
		}else
		{
...
bool CEnemyDefendTacticArea::isInAnnulus(CGeoPoint & enemyCarPos)
{
 ...
 ////内圆判断
	...
	if (abs(enemyCarPos.y()) < Param::Field::PENALTY_AREA_L / 2.0)
	{
		if ((enemyCarPos - theirGoal ).mod() <  ( queryParamByName("data\\ssl\\params\\params.xml", "PENALTY_AREA_DEPTH") + buffer ))
		{
			inOuterCicle = true;
		}else
		{
