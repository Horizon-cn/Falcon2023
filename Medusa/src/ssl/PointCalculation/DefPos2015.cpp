...
void CDefPos2015::generatePos(const CVisionModule* pVision) {
	...
	posSide RtargetSide = posSide::POS_SIDE_MIDDLE;
	...
	if (leftBack != 0 && rightBack != 0) {
		if (Utils::InOurPenaltyArea(pVision->Ball().Pos(), Param::Vehicle::V2::PLAYER_SIZE / 2)) {
			// left,right防破防的车
		...
		GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(-251, 400), "single back");
		if (posSide::POS_SIDE_RIGHT == RtargetSide) {
			...
			if (goalieFirst == true) {
				...
				if (BallShooted) {
					GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(-251, 361), "Ball Shooted");
					RsinglePoint = calcDefenderPointV3(_RdefendTarget, RBallVel.dir(), defSide, 0);
				}
