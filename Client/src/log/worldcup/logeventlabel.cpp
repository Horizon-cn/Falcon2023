        ...
        if (!isDribbling) {
            ...
            if (frames[frameIndex].state & BALL_POSSESSION_UNKNOWN) {
                ...
                if (frames[frameIndex].state & BALL_POSSESSION_BLUE)
                    _dribblelLabel->set_team(BLUE_TEAM);
                else
                    _dribblelLabel->set_team(YELLOW_TEAM);
...
LabelerFrameGroup LogEventLabel::readFrame(int index) {
    ...
    messageLength = (quint32* ) malloc (sizeof(quint32));
...
bool LogEventLabel::loadFile(QString _fileName) {
    ...
    //header
    quint32* formatVesion;
    formatVesion = (quint32* ) malloc (sizeof(quint32));
    buf = (char*) malloc (sizeof(char) * 22);
    in.read(buf, 20);
    formatVesion = (quint32 *)&buf[16];
    //metaData
    in.seekg(-4, in.end);
    quint32* metaDataLength;
    metaDataLength = (quint32* ) malloc (sizeof(quint32));
    buf = (char*) malloc (sizeof(char) * 4);
    in.read(buf, 4);
    metaDataLength = (quint32 *)&buf[0];
