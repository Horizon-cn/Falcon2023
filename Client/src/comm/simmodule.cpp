...
void SimModule::readBlueData() {
    ...
    while(true){
        ...
        while(blueReceiveSocket.state() == QUdpSocket::BoundState && blueReceiveSocket.hasPendingDatagrams()) {
            //Owl::Protocol::Robots_Status robotsPacket;
...
//void SimModule::sendSim(int t, Owl::Protocol::Robots_Command& command) {
...
void SimModule::sendEmptySim() {
    ...
    QByteArray data(size, 0);
    grsim_packet.SerializeToArray(data.data(), data.size());
    command_socket.writeDatagram(data, size, QHostAddress(cpm->local_address), Owl::Athena::SIM_SEND);
}
**/
}
