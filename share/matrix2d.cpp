...
/* CONVERT FUNCTIONS (By SRC)
 ********************************/

string Matrix2d::to_str(){
    string str = std::to_string(_row);
    str += std::to_string(_column);
    for (int i = 0; i < _row; ++i) {
        for (int j = 0; j < _column; ++j) {
            str += std::to_string(_value[i][j]);
        }
    }
    return str;
}

void Matrix2d::from_string(string str){
    _row = str[0] - '0';
    _column = str[1] - '0';
    int l = str.length();
    if(l-2 != _row*_column){
    ...
    for (int i = 0; i < _row; ++i) {
        for (int j = 0; j < _column; ++j) {
            _value[i][j] = str[i*_column+j];
        }
    }
}

void Matrix2d::from_string(const char* str){
    _row = str[0] - '0';
    _column = str[1] - '0';
    int l = strlen(str);
    if(l-2 != _row*_column){
