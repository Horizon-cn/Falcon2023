    ...
    : QMainWindow(parent)
{
    ...
    glwidget->ssl->visionServer2 = visionServer2;
    ...
    QObject::connect(configwidget->v_NumOfCam.get(), SIGNAL(wasEdited(VarPtr)), this, SLOT(restartSimulator()));
    QObject::connect(configwidget->v_DesiredFPS.get(), SIGNAL(wasEdited(VarPtr)), this, SLOT(changeTimer()));
    QObject::connect(configwidget->v_LoginName.get(), SIGNAL(wasEdited(VarPtr)), this, SLOT(restartSimulator()));
	...
	QObject::connect(configwidget->v_EnableYellowSim.get(), SIGNAL(wasEdited(VarPtr)), this, SLOT(restartSimulator()));
	QObject::connect(configwidget->v_EnableWallSim.get(), SIGNAL(wasEdited(VarPtr)), this, SLOT(restartSimulator()));
    ...
    QObject::connect(configwidget->v_VisionMulticastAddr2.get(), SIGNAL(wasEdited(VarPtr)), this, SLOT(reconnectVisionSocket()));
    QObject::connect(configwidget->v_VisionMulticastPort2.get(), SIGNAL(wasEdited(VarPtr)), this, SLOT(reconnectVisionSocket()));
...
{
	//cout << glwidget->getFPS() << endl;
	//xjw, 先调用一次step(),改变GraphicDisabled值,-H模式才能用
	//改GraphicDisabled初始化的值也可以用-H模式，但普通模式下无法加载图片
	if (first_time) {
		glwidget->step(); 
		first_time = false;
	}
	...
	if (clock() - t > 300) {
		if (last_recv_cnt == 0);
		else if (last_recv_cnt == recv_cnt) {
			logStatus("no more vision, stop robots", QColor("green"));
			for (int i = 0; i < configwidget->Robots_Count()*2; i++) {
				glwidget->ssl->robots[i]->setSpeed(0, 0, 0);
			}
			last_recv_cnt = recv_cnt = 0;
		}
...
{        
    delete glwidget->ssl;
    if(configwidget->EnableYellowSim()) glwidget->ssl = new SSLWorld(glwidget,glwidget->cfg,glwidget->forms[2],glwidget->forms[2]);
	else glwidget->ssl = new SSLWorld(glwidget, glwidget->cfg, glwidget->forms[2], glwidget->forms[4]);
    glwidget->ssl->glinit();
    glwidget->ssl->visionServer = visionServer;
    glwidget->ssl->visionServer2 = visionServer2;
    glwidget->ssl->commandSocket = commandSocket;
...
void MainWindow::reconnectCommandSocket()
{
	...
	if (commandSocket->bind(QHostAddress::AnyIPv4, configwidget->CommandListenPort(), QUdpSocket::ShareAddress))
		logStatus(QString("Command listen port binded on: %1").arg(configwidget->CommandListenPort()), QColor("green"));

	if (commandSocket->joinMulticastGroup(QHostAddress("224.5.23.3")))
		logStatus(QString("Command listen socket join muticastgroup."), QColor("green"));
...
void MainWindow::reconnectVisionSocket()
{
    ...
    if (visionServer2 == NULL) {
        visionServer2 = new RoboCupSSLServer(this);
    }
    visionServer->change_address(configwidget->VisionMulticastAddr());
    visionServer->change_port(configwidget->VisionMulticastPort());
    visionServer2->change_address(configwidget->VisionMulticastAddr2());
    visionServer2->change_port(configwidget->VisionMulticastPort2());
    logStatus(QString("Vision server connected on: %1").arg(configwidget->VisionMulticastPort()),QColor("green"));
    logStatus(QString("Vision server connected on: %1").arg(configwidget->VisionMulticastPort2()),QColor("green"));
}
