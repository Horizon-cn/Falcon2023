...
cfg(_cfg)
{
    ...
    if (type==3) // outside field
    {
		...
		if (cfg->Division() == "Division B") {
			for (int i = 0; i < MAX_ROBOT_COUNT; i++) teamPosY[i] = -4.0;
		}
		else if (cfg->Division() == "Division A") {
			for (int i = 0; i < MAX_ROBOT_COUNT; i++) teamPosY[i] = -5.3;
		}
        setAll(teamPosX,teamPosY);
    }
    if (type==4)
    ...
    if (type==-1) // outside
    {
		...
		dReal teamPosY[MAX_ROBOT_COUNT];
		if (cfg->Division() == "Division B") {
			for (int i = 0; i < MAX_ROBOT_COUNT; i++) teamPosY[i] = -3.4;
		}
		else if (cfg->Division() == "Division A") {
			for (int i = 0; i < MAX_ROBOT_COUNT; i++) teamPosY[i] = -4.8;
		}
        setAll(teamPosX,teamPosY);
    }
