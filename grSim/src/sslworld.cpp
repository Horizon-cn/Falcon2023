    ...
    : QObject(parent)
{
	...
	else std::cout << "yellow simulator : enabled" << std::endl;
	if (closeWallSimulation) std::cout << "wall simulator : disabled" << std::endl;
	else std::cout << "wall simulator : enabled" << std::endl;
    std::cout<<"the number of cameras : "<<cfg->NumOfCam()<<endl;
    m_parent = parent;
    show3DCursor = false;
    updatedCursor = false;
    framenum = 0;
    ...
    const double increment = cfg->Field_Margin() + thick / 2; //cfg->Field_Margin() + cfg->Field_Referee_Margin() + thick / 2;
	...
	for (int k = 0; k < cfg->Robots_Count(); k++) {
		robots[k + cfg->Robots_Count()] = new Robot(p, ball, cfg, form2->x[k], form2->y[k], ROBOT_START_Z(cfg), ROBOT_GRAY, ROBOT_GRAY, ROBOT_GRAY, k + cfg->Robots_Count() + 1, wheeltexid, -1);
		if (closeYellowSimulation)robots[k + cfg->Robots_Count()]->on = false;
	}
	p->initAllObjects();
    ...
    for (int k=0; k<(2-closeYellowSimulation)*cfg->Robots_Count() ;k++)
	{
    ...
    for (int k = 0; k < (2-closeYellowSimulation)*cfg->Robots_Count(); k++)
    {
        ...
        for (int j = k + 1; j < (2-closeYellowSimulation)*cfg->Robots_Count(); j++)
        {            
            if (k != j)
            {
...
void SSLWorld::step(dReal dt)
{
    ...
    for (int k=0;k<cfg->Robots_Count() * (2)/* - closeYellowSimulation)*/ ;k++)
    {
    ...
    for (int k=0;k<cfg->Robots_Count() * (2 - closeYellowSimulation) ;k++)
    {
		...
		double r = robots[k]->cfg->robotSettings.RobotRadius;
		double x, y;
		robots[k]->getXY(x, y);
		if (closeWallSimulation && (touchGoal(x, y, r) || OutofField(x, y, r))) robots[k]->setSpeed(0, 0, 0);
    }
	//if (g->isGraphicsEnabled()) {
		p->draw();
		//g->drawSkybox(31,32,33,34,35,36);
		g->drawSkybox(
			4 * cfg->Robots_Count() + 6 + 1, //31 for 6 robot
            ...
            if (packet.has_commands())
            {
                ...
                for (int i=0;i<packet.commands().robot_commands_size();i++)
                {
					if (!packet.commands().robot_commands(i).has_id()) {
						robots[robotIndex(packet.commands().robot_commands(i).id(), team)]->setSpeed(0, 0, 0);
						continue;
					}
                    int k = packet.commands().robot_commands(i).id();
                    int id = robotIndex(k, team);
                    if ((id < 0) || (id >= cfg->Robots_Count()*(2-closeYellowSimulation))) continue;
                    bool wheels = false;
...
bool SSLWorld::visibleInCam(int id, double x, double y)
{
    ...
    switch (cfg->NumOfCam()) {
        ...
        case 2: switch (id) {
                    case 0: return (x < cfg->overlap());
                    case 1: return (x > -cfg->overlap());
                }
        case 4: switch (id) {
                    case 0: return (x < cfg->overlap() && y < cfg->overlap());
                    case 1: return (x < cfg->overlap() && y > -cfg->overlap());
                    case 2: return (x > -cfg->overlap() && y < cfg->overlap());
                    case 3: return (x > -cfg->overlap() && y > -cfg->overlap());
                }
        case 8: switch (id) {
...
void SSLWorld::sendVisionBuffer()
{
    ...
    if(cfg->NumOfCam()==1) 
        ...
        for(int i = 0; i < cfg->NumOfCam(); i++) {
            sendQueue.push_back(new SendingPacket(generatePacket(i),t+(i%(cfg->NumOfCam()/2))));
        }
...
bool SSLWorld::OutofField(dReal x, dReal y, double r) {
	if (x > walls[2]->getX() - cfg->Wall_Thickness() - r) return 1;
	if (x < walls[3]->getX() + cfg->Wall_Thickness() + r) return 1;
	if (y > walls[0]->getY() - cfg->Wall_Thickness() - r) return 1;
	if (y < walls[1]->getY() + cfg->Wall_Thickness() + r) return 1;
	return 0;
}
bool SSLWorld::touchGoal(dReal x, dReal y, double r) {
	if (fabs(x - walls[4]->getX()) < cfg->Wall_Thickness() + r && fabs(y - walls[4]->getY()) < cfg->Goal_Width()/2 + r) return 1;
	if (fabs(x - walls[7]->getX()) < cfg->Wall_Thickness() + r && fabs(y - walls[7]->getY()) < cfg->Goal_Width()/2 + r) return 1;
	if (fabs(x - walls[5]->getX()) < cfg->Goal_Thickness() + cfg->Goal_Depth() / 2 + r && fabs(y - walls[5]->getY()) < cfg->Goal_Thickness() + r) return 1;
	if (fabs(x - walls[6]->getX()) < cfg->Goal_Thickness() + cfg->Goal_Depth() / 2 + r && fabs(y - walls[6]->getY()) < cfg->Goal_Thickness() + r) return 1;
...
cfg(_cfg)
{
    ...
    if (type==3) // outside field
    {
		...
		if (cfg->Division() == "Division B") {
            for (int i = 0; i <= 11; i++) teamPosY[i] = -4.00;
            for (int i = 12; i < MAX_ROBOT_COUNT; i++) teamPosY[i] = -4.40;
		}
		else if (cfg->Division() == "Division A") {
            for (int i = 0; i <= 11; i++) teamPosY[i] = -5.30;
            for (int i = 12; i < MAX_ROBOT_COUNT; i++) teamPosY[i] = -5.70;
		}
        setAll(teamPosX,teamPosY);
    }
    if (type==4)
    ...
    if (type==-1) // outside
    {
		...
		dReal teamPosY[MAX_ROBOT_COUNT];
		if (cfg->Division() == "Division B") {
            for (int i = 0; i <= 11; i++) teamPosY[i] = -3.40;
            for (int i = 12; i < MAX_ROBOT_COUNT; i++) teamPosY[i] = -3.20;
		}
		else if (cfg->Division() == "Division A") {
            for (int i = 0; i <= 11; i++) teamPosY[i] = -4.80;
            for (int i = 12; i < MAX_ROBOT_COUNT; i++) teamPosY[i] = -4.60;
		}
        setAll(teamPosX,teamPosY);
    }
